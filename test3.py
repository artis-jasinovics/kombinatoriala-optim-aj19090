...
rooms = [
    Room(1, 2, False, False),  
    Room(2, 3, True, True)     
]

employees = [Employee(i, []) for i in range(1, 4)]

project1 = Project(1, [employees[i].employee_id for i in [0, 1]])
project2 = Project(2, [employees[i].employee_id for i in [1, 2]])

for emp in [employees[i] for i in [0, 1]]:
    emp.projects.append(project1.project_id)
for emp in [employees[i] for i in [1, 2]]:
...
meeting_schedule.schedule_meeting(meetings[0], rooms[1], 0)
